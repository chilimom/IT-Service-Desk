...
function TicketDetails() {
  ...
  // Xác định ticket có phải là Maintenance hay không (dùng chung cho toàn component)
  const isMaintenance = useMemo(() => {
    if (!ticket) return false
    return ticket.categoryName?.toLowerCase().includes('bảo trì') || false
  }, [ticket])
  ...
  useEffect(() => {
    async function loadTicketDetails() {
      try {
        ...
        // Xác định loại ticket dựa trên categoryName
        const isMaintenanceTicket = ticketData?.categoryName?.toLowerCase().includes('bảo trì')
        const ticketMaintenanceCategory = getMaintenanceCategory(ticketData)
        
        setForm({
          type: isMaintenanceTicket ? 'Maintenance' : 'IT',
          factory: ticketData.factory || '',
          maintenanceCategory: ticketMaintenanceCategory?.code || 'PM01',
          title: ticketData.title || '',
          description: ticketData.description || '',
  ...
  const handleSave = async (event) => {
    ...
    try {
      ...
      const updatedIsMaintenance = updatedTicket?.categoryName?.toLowerCase().includes('bảo trì')
      const updatedMaintenanceCategory = getMaintenanceCategory(updatedTicket)
      
      setForm({
        type: updatedIsMaintenance ? 'Maintenance' : 'IT',
        factory: updatedTicket.factory || '',
        maintenanceCategory: updatedMaintenanceCategory?.code || 'PM01',
        title: updatedTicket.title || '',
        description: updatedTicket.description || '',
